...
public interface IFavoriteRepository extends JpaRepository<Favorite, Long> {
    ...
    @Query(value = "select new com.Travel.butler.vo.UserVo(v.innCode, v.account,count(1)) from Favorite v where v.innCode = :inncode and v.flag = 1 group by innCode, account")
    public List<UserVo> findFavoriteCount(@Param("inncode") String inncode);

    @Query(value = "select new com.Travel.butler.vo.UserVo(v.innCode, v.account,count(*)) from Favorite v where v.innCode = :inncode and v.openid = :openid and v.flag = 1 group by innCode, account")
