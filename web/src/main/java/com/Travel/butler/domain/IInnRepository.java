...
public interface IInnRepository  extends JpaRepository<Inn, Long> {
    ...
    @Query("select new com.Travel.butler.vo.InnVo(t.innCode, t.innName) from Inn t where t.flag = 1")
    ...
    @Query("select new com.Travel.butler.vo.InnVo(t.innCode, t.innName, t.longitude, t.latitude) from Inn t where t.flag = 1 and t.longitude is not null and t.latitude is not null and  EXISTS (SELECT 1 from Sysuserdetail ud where ud.innCode = t.innCode) ")
